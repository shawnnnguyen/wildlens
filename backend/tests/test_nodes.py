...
from wildlens.nodes.check_relevance.prompts import _OFF_TOPIC_EXEMPLARS, _ON_TOPIC_EXEMPLARS
...
def test_enqueue_enrichment_gives_each_web_doc_a_distinct_section():
    ...
    from langchain_core.documents import Document
    from wildlens.nodes.retrieve_information.node import _enqueue_enrichment
    from wildlens.rag import _EnsembleRetriever
...
def test_format_fact_labels_web_enriched_as_web_not_guidebook():
    ...
    from langchain_core.documents import Document
    from wildlens.nodes.retrieve_information.node import _format_fact
...
def test_generate_audio_calls_synthesise(tmp_path):
    ...
    with patch("wildlens.nodes.generate_audio.node.synthesise_audio", return_value=str(tmp_path / "test.mp3")) as mock_tts:
        result = node_generate_audio(state)
